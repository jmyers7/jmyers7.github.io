...
6. <a href="#end" style="font-family: 'Lora', serif; font-size: 1em;">References and footnotes</a>
...
<div style='overflow: scroll'>
\begin{equation}\notag
\pi(y\|x) = \frac{\ell(y\|x) \pi(y)}{\int_\caly \ell(y'\|x) \pi(y') \ \nu(\text{d}y').}
\end{equation}
</div>
If we write the conditional density $f(x\|y)$ in place of the likelihood, then this becomes:
<div style='overflow: scroll'>
\begin{equation}\notag
...
<div class="highlight-box">
**Maximum a Posteriori Estimation.** To predict the label of a given feature $x\in \calx$, we simply choose the label $y\in \caly$ that maximizes the probability density $\pi(y\|x).$
</div>
