...
>>> first_product = np.tensordot(np.tensordot(v, w, axes=0),
                                 u, axes=0)
>>> print(first_product))
...
>>> u = np.array([8, 9, 10, 11, 12])
>>> second_product = \ 
        np.tensordot(v, np.tensordot(w, u, axes=0), axes=0)
...
If we fix orderd bases of each of the tensor factors, then every rank-$n$ tensor $T\in V_1\otimes \cdots \otimes V_n$ may be written as
...
```py
>>> linear_combo = 0
>>> for (alpha, beta, gamma, delta, epsilon) in\
>>>         product(range(4), range(5), range(6), range(7), range(8)):
>>>     basis_tensor = t(e(4, alpha), e(5, beta),\
>>>                      e(6, gamma), e(7, delta), e(8, epsilon))
>>>     linear_combo += S[alpha, beta] *\
>>>                    T[gamma, delta, epsilon] * basis_tensor
```
