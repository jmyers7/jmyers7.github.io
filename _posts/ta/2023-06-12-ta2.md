...
If for each nonempty subset $A\subseteq\sigma(T)$ we define

\begin{equation}\notag
E(A) = \sum_{\lambda \in \sigma(T) \cap A} P_\lambda,
\end{equation}

then the function $E: \mfP(\sigma(T)) \to \calb(\calh)$ is a spectral measure.
</div>

To complete the definition of $E$ we must, of course, also set $E(\emptyset) = \zero$. To prove the theorem, we must check all the defining axioms of a spectral measure:
