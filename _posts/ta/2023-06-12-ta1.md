...
<h1 class="accent-color">basic definitions and uniform structures</h1>
</div>
...
1. <a href="#defs" style="font-family: 'Lora', serif; font-size: 1em;">Basic definitions</a>
2. <a href="#uniform" style="font-family: 'Lora', serif; font-size: 1em;">Uniform structures</a>
3. <a href="#converge" style="font-family: 'Lora', serif; font-size: 1em;">Convergence and completeness</a>
...
<div class="highlight-box2">
**Exercise.** Prove the theorem. If you have difficulties, see Howe's article referenced in the section on Further Reading.
</div>

There are versions of this result for vector spaces and algebras:

<div class="highlight-box">
**Theorem (When do homogeneous topologies yield topological algebraic structures?).**

1. Let $V$ be a $k$-vector space whose underlying additive abelian group is equipped with a homogeneous topology. Then $V$ is a topological $k$-vector space if and only if the subtraction map
	\begin{equation}\label{yupper-eqn}
	V \times V \to V, \quad (v,w) \mapsto v-w,
	\end{equation}
is continuous at $(0,0)$, and the scaling map
	\begin{equation}\label{yupper2-eqn}
	k \times V \to V, \quad (\lambda,v) \mapsto \lambda v,
	\end{equation}
is continuous at $(0,0)$.
2. Let $A$ be a $k$-algebra whose underlying additive abelian group is equipped with a homogeneous topology. Then $A$ is a topological $k$-algebra if and only if the subtraction map
	\begin{equation}\notag
	A \times A \to A, \quad (a,b) \mapsto a-b,
	\end{equation}
is continuous at $(0,0)$, the scaling map
	\begin{equation}\notag
	k \times A \to A, \quad (\lambda,a) \mapsto \lambda a,
	\end{equation}
is continuous at $(0,0)$, and the multiplication map
	\begin{equation}\label{yupper3-eqn}
	A \times A \to A, \quad (a,b) \mapsto ab,
	\end{equation}
is continuous at $(0,0)$.
</div>
...
A *normed $k$-vector space* is a $k$-vector space equipped with a norm $\norm{\cdot}$. A normed $k$-vector space is called a *Banach space* if it is complete in the metric induced by the norm (see immediately below).
</div>

Every normed vector space $V$ is a topological space, equipped with the metric topology induced by

\begin{equation}\notag
\text{distance}(v,w) \defeq \norm{v-w}, \quad v,w\in V.
\end{equation}

While it is quite straightforward to prove directly that a normed vector space is a topological vector space, it is perhaps instructive to use some of the theory that we've built up in this section.

<div class="highlight-box">
**Theorem.** Every normed $k$-vector space $V$ is a topological vector space.
</div>

For the proof, let's let $B(v;\dev)$ denote the open $\dev$-ball centered at $v\in V$, for $\dev>0$:

\begin{equation}\notag
B(v;\dev) = \left\\{ w \in V \mid \norm{w-v} < \dev \right\\}.
\end{equation}

To prove that the metric topology is homogeneous, we need to show that each left-translation map $L_v:V \to V$ is continuous. For this, we fix $w\in V$ and $\dev>0$, and we need to find a $\delta>0$ such that

\begin{equation}\notag
L_v \left( B(w;\delta) \right) \subseteq B(v+w; \dev).
\end{equation}

But this is completely trivial because, as you may check, we can just take $\delta = \dev$.

Now that we know the metric topology is homogeneous, to prove $V$ is a topological vector space it will suffice to show that the subtraction map \eqref{yupper-eqn} is continuous at $(0,0)$, and that the scaling map \eqref{yupper2-eqn} is continuous at $(0,0)$. But this is precisely where the homogeneity property of the norm and the triangle inequality come into play. I will leave you to work out the (straightforward) details. Q.E.D.
