...
# Create a sequence of possible values for X (hours studied)
xs = range(1, n + 1)
...
# This is a mixture of Beta distributions, weighted by the probability mass fxs for each x.
def fy(y):
...
axes[0].set_xticks(range(1, n + 1))  # Set x-ticks to match possible values of x
...
axes[1].set_xticks(range(1, n + 1))  # Set x-ticks to match possible values of x
...
# Define the conditional density f(y|x) as the Beta(x, 3) PDF
def cond(y, x):
    return beta.pdf(x=y, a=x, b=3)


# Define the integrand for KL divergence between two Beta distributions:
# cond(y, x1) * log(cond(y, x1) / cond(y, x2))
def integrand(y, x1, x2):
    return cond(y, x1) * np.log(cond(y, x1) / cond(y, x2))
...
for x, ax in enumerate(axes):
    ...
    ax.plot(grid, cond(grid, 1), color=blue, zorder=2, label="x = 1")
    ax.fill_between(grid, cond(grid, 1), zorder=2, color=blue, alpha=0.1)
    
    # Plot f(y|x=x+2) in yellow
    ax.plot(grid, cond(grid, x + 2), color=yellow, zorder=2, label=f"x = {x + 2}")
    ax.fill_between(grid, cond(grid, x + 2), zorder=2, color=yellow, alpha=0.1)
...
# function `cond(y, x)`, the marginal density of Y is
# the function `fy(y)`, and the marginal probability masses
# of X are contained in a NumPy array called `fxs`.

# Compute the true joint density f(x, y) = P(X=x) * f(y|x)
def fxy(x, y):
    return fxs[x - 1] * cond(y, x)

# Compute the product of marginals: P(X=x) * f(y)
def prod(x, y):
    return fxs[x - 1] * fy(y)

# Define the integrand for mutual information:
# f(x, y) * log(f(x, y) / (P(X=x) * f(y)))
def integrand(x, y):
...
funcs = [lambda y, x=x: -cond(y, x) * np.log(cond(y, x)) for x in range(1, 7)]
...
diff_entropy - sum([cond_entropy * fx for cond_entropy, fx in zip(cond_entropies, fxs)])
