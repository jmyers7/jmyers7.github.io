...
If you’d like to follow along with the code examples in this post, please see the code block below for the usual imports and setup.
...
class RV:
    ...
    def __init__(
        ...
        if support is not None and density is not None:
            self.pdf_array = np.array([density(x) for x in support])
        else:
            self.pdf_array = None
...
ax.bar(X.support, X.pdf_array, width=0.4, zorder=2)
...
Y = RV(
    ...
    density=fy,
    conditional_density=fy_given_x,
)
...
ax.plot(Y.support, Y.pdf_array)

# Shade the area under the density curve for visual emphasis.
ax.fill_between(Y.support, Y.pdf_array, zorder=2, alpha=0.1)
...
for x in X.support:
    ...
    joint_vals = 1.7 * Y.cond_pdf(Y.support, x) * X.pdf(x)
...
axes[0].bar(X.support, X.pdf_array, width=0.4, zorder=2)
...
axes[1].bar(X.support, -np.log(X.pdf_array), width=0.4, zorder=2)
...
axes[0].plot(Y.support, Y.pdf_array)
...
axes[1].plot(Y.support[:-1], -np.log(Y.pdf_array[:-1]))
...
print(f"\nThe entropy H(X) is {entropy(X.pdf_array):.3f}.")
```
...
for x, ax in zip(range(2, 7), axes):
    ...
    ax.plot(Y.support, Y.cond_pdf(Y.support, 1), color=blue, zorder=2, label="x = 1")
    ax.fill_between(
        Y.support, Y.cond_pdf(Y.support, 1), zorder=2, color=blue, alpha=0.1
    )

    # Plot f(y|x) in yellow
    ax.plot(
        Y.support, Y.cond_pdf(Y.support, x), color=yellow, zorder=2, label=f"x = {x}"
    )
    ax.fill_between(
        Y.support, Y.cond_pdf(Y.support, x), zorder=2, color=yellow, alpha=0.1
    )
