---
title: "Entropy, surprise & information"
date: "2025-10-12"
...
This post is the first in a series on *information*. Future posts will examine other ways the concept of information manifests, for example through $\sigma$-algebras, and how these ideas connect to the probabilistic models commonly used in machine learning. I have discussed information theory previously in a [chapter](https://johnmyers-phd.com/book/chapters/09-info-theory.html){target="_blank"} of my book; while some material overlaps with that chapter, this series also introduces many new perspectives and examples.
...
titles = ['$f(\\omega)$', '$g(\\omega)$', '$\\frac{f(\\omega)}{g(\\omega)}$', '$\\log\\left(\\frac{f(\\omega)}{g(\\omega)}\\right)$']
...
# the conditional random variable Y|x=x is a
# Beta(x, 3) random variable. We implement its
# conditional density
...
where $f(x,y)$ is the true joint mass-density function and $f(x)$ and $f(y)$ are the marginal mass and densities, respectively. We implement this formula directly in `Python`, using the `quad` method in the `stats` submodule of `SciPy` for integration:
...
def plot_multivar_norm(ax, muX, muY, sigmaX, sigmaY, x, y, labels=False):
  ...
  grid = np.dstack((x, y))
  z = U.pdf(grid)
  contour = ax.contour(x, y, z, colors=yellow, alpha=0.3)
  if labels:
    ax.clabel(contour, inline=True, fontsize=8)
  
def plot_conditional(ax, muX, muY, sigmaX, sigmaY, rho, y_mesh, x_obs, magnification_factor=1):
...
rhos = [0.5, 0.85]
x_obs = [-1, 0, 1]
x, y = np.mgrid[-2.1:2.1:0.01, -5:5:0.01]
y_mesh = np.linspace(-5, 5, num=250)

fig, axes = plt.subplots(ncols=3, figsize=(8, 4), sharey=True, gridspec_kw={'width_ratios': [1, 4, 4]})
...
for rho, ax in zip(rhos, axes[1:]):
  ...
  for x_ob in x_obs:
    plot_conditional(ax, muX, muY, sigmaX, sigmaY, rho, y_mesh, x_obs=x_ob, magnification_factor=3)
