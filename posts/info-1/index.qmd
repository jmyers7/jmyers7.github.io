...
# Set the number of possible study hours (1 through 6)
n = 6

# Compute the probability mass function for X using a Poisson distribution with mean 3
fx_array = poisson.pmf(range(1, n + 1), mu=3)

# Normalize the probabilities so they sum to 1 (ensures a valid probability distribution)
fx_array /= fx_array.sum()


# Define the probability mass function f(x) of X
def fx(x):
    return fx_array[x - 1]
...
ax.bar(range(1, n + 1), fx_array, width=0.4, zorder=2)
...
# Define the conditional density function f(y|x) for exam scores.
# This returns the value of the Beta(x, 3) density at y, for a given x.
def fy_given_x(y, x):
    return beta.pdf(y, a=x, b=3)

# Define the marginal density function fy(y) for exam scores.
# This is a mixture of Beta distributions, weighted by the probability mass fx(x) for each x.
def fy(y):
    return sum([fy_given_x(y, x) * fx(x) for x in range(1, 7)])

# Create a new matplotlib figure and axis for plotting the marginal density.
_, ax = plt.subplots(figsize=(6, 4))

# Create a grid of y values between 0 and 1 for plotting the density.
grid = np.linspace(0, 1, num=250)

# Plot the marginal density fy(y) over the grid.
...
# Loop over each possible value of x and its probability fx
for x in range(1, 7):
    # Compute the joint density values for each y in the grid, scaled for visualization
    joint_vals = 1.7 * fy_given_x(grid, x) * fx(x)

    # Fill the area between the baseline (x) and the curve (x + joint_vals) for ridgeline effect
    ax.fill_between(
        grid, x, x + joint_vals, color=conditional_colors[x - 1], zorder=2, alpha=0.1
    )
...
# Create a grid of y values between 0 and 1 for plotting
grid = np.linspace(0, 1, num=250)
...
print(f"\nThe entropy H(X) is {entropy(fx_array):.3f}.")
```
...
# The marginal probability density function of Y is implemented as
# the function `fy`
...
def integrand(y, x1, x2):
    return fy_given_x(y, x1) * np.log(fy_given_x(y, x1) / fy_given_x(y, x2))


# Compute KL divergence D((Y|X=1) || (Y|X=x+2)) for x = 0,1,2,3,4 (i.e., x2 = 2,3,4,5,6)
KL_div = np.empty(5)
for x in range(5):
...
for x, ax in enumerate(axes):
    ...
    ax.plot(grid, fy_given_x(grid, 1), color=blue, zorder=2, label="x = 1")
    ax.fill_between(grid, fy_given_x(grid, 1), zorder=2, color=blue, alpha=0.1)

    # Plot f(y|x=x+2) in yellow
    ax.plot(grid, fy_given_x(grid, x + 2), color=yellow, zorder=2, label=f"x = {x + 2}")
    ax.fill_between(grid, fy_given_x(grid, x + 2), zorder=2, color=yellow, alpha=0.1)
...
# The conditional density of Y|X=x is implemented as the
# function `fy_given_x(y, x)`, the marginal density of Y is
# the function `fy(y)`, and the marginal mass of X
# is the function `fx(x)`

# Compute the true joint density f(x, y) = f(x) * f(y|x)
def fxy(x, y):
    return fx(x) * fy_given_x(y, x)

# Compute the product of marginals: f(x) * f(y)
def prod(x, y):
    return fx(x) * fy(y)
...
def integrand(x, y):
    return fxy(x, y) * np.log(fxy(x, y) / prod(x, y))
...
funcs = [lambda y, x=x: -fy_given_x(y, x) * np.log(fy_given_x(y, x)) for x in range(1, 7)]
...
# Compute H(Y) - sum_x P(X=x) * H(Y|X=x), which equals the mutual information I(X, Y)
diff_entropy - sum([cond_entropy * fx for cond_entropy, fx in zip(cond_entropies, fx_array)])
