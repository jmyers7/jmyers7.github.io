...
Omega <- Omega %>%
  mutate(
    p = (theta^Y) * ((1 - theta)^(3 - Y)) # Probability of each outcome based on number of heads
  )

# Print the sample space with probabilities
kable(
  Omega,
...
For those readers who are familiar with abstract integration theory, we note that the formula ([-@eq-def-entropy]) can be expressed as a Lebesgue integral. Indeed, if we write $\pi: \Omega \to \Omega/\sim$ for the natural projection map onto the quotient of $\Omega$ by the $\calG$-equivalence relation ([-@eq-equiv-rel]), then the entropy of $\calG$ is given by the Lebesgue integral

$$
H(\calG) = \int_{\Omega/\sim} s(B) \ \pi_\ast P(dB),
$$

where $s:\Omega/{\sim}  \to \bbr$ is the surprisal function given by

$$
s(B) = -\log{P(B)},
$$

and $\pi_\ast P$ is the pushforward measure of $P$ under $\pi$ defined on the power set of the quotient $\Omega/\sim$. By the substitution rule, we also have
...
# Calculate probabilties on the atoms of F1
F1_probs <- Omega %>%
  group_by(Ai) %>%
  summarise(p = sum(p))

# Print the probabilities for the atoms of F1
kable(
  F1_probs,
...
# Print the probabilities for the atoms of F2
kable(
  F2_probs,
...
# Print the probabilities for the atoms of F3
kable(
  F3_probs,
...
# Insert the entropies into a data frame
marginal_entropies <- tibble(
  entropy = c("H(F1)", "H(F2)", "H(F3)"),
  value = c(H_F1, H_F2, H_F3)
)

# Print the three marginal entropies
kable(
  marginal_entropies,
...
ggplot(marginal_entropies, aes(x = entropy, y = value)) +
  geom_col(fill = yellow, width = 0.4) +
  labs(
    title = "entropy increases under refinement",
    x = NULL,
    y = "entropy"
  )
...
The intuition for this definition is as follows. The entropy $H(\calH)$ measures the uncertainty in $\calH$, while $H(\calG \cup \calH)$ measures the uncertainty in the combined information of $\calG$ and $\calH$. The difference between these two quantities, $H(\calG \mid \calH)$, thus represents the uncertainty that remains in $\calG$ once we know everything in $\calH$.
...
conditional_entropies <- tibble(
  entropy = c("H(F3 | F1)", "H(F3 | F2)", "H(F3 | F3)"),
  value = c(H_F3 - H_F1, H_F3 - H_F2, H_F3 - H_F3)
)

# Print the three conditional entropies
kable(
...
mutual_infos <- tibble(
  entropy = c("I(F3, F1)", "I(F3, F2)", "I(F3, F3)"),
  value = c(H_F1, H_F2, H_F3)
)

# Print the three mutual informations
kable(
  mutual_infos,
...
decomposition <- tibble(
  algebra = rep(c("i=1", "i=2", "i=3"), each = 2),
  component = rep(c("mutual information", "conditional entropy"), 3),
  value = c(H_F1, H_F3 - H_F1, H_F2, H_F3 - H_F2, H_F3, 0)
)

ggplot(decomposition, aes(x = algebra, y = value, fill = component)) +
  geom_col(width = 0.4) +
  scale_fill_manual(
    name = NULL,
