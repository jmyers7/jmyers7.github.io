...
## Orthonormal bases and partial derivatives
...
The importance of the partial derivatives is that they appear as the entries in matrix representations of derivatives:
...
$$
\begin{bmatrix}
\displaystyle\sum_j v_j \frac{\partial f_1}{\partial e_j}(x) \\
\vdots \\
\displaystyle\sum_j v_j \frac{\partial f_m}{\partial e_j}(x)
\end{bmatrix}=\begin{bmatrix} \displaystyle\frac{\partial f_1}{\partial e_1}(x) & \cdots & \displaystyle\frac{\partial f_1}{\partial e_n}(x) \\ \vdots & \ddots & \vdots \\ \displaystyle\frac{\partial f_m}{\partial e_1}(x) & \cdots & \displaystyle\frac{\partial f_m}{\partial e_n}(x) \end{bmatrix} \begin{bmatrix} v_1 \\ \vdots \\ v_n \end{bmatrix}.
$$

## Direct sums and partial derivatives

The construction of the partial derivatives in the previous section is a special case of a more general construction described in this section. To set up, suppose that $\calh = \bigoplus_j \calh_j$ and $\calk = \bigoplus_i \calk_i$ are orthogonal direct sums of finite-dimensional Hilbert spaces, where the indices range over finite sets. Write

$$
\iota_j^\calh: \calh_j \to \calh, \quad \pi_i^\calk: \calk \to \calk_i
$$

and

$$
\pi_i^\calh: \calh \to \calh_i, \quad \iota_i^\calk: \calk_i \to \calk,
$$

for the natural inclusion and projection maps, respectively. If $f: \calh \to \calk$ is a function, then we may define the component functions

$$
f_{ij}: \calh_j \to \calk_i, \quad f_{ij} = \pi_i^\calk \circ f \circ \iota_j^\calh,
$$

for each $i$ and $j$. Note then that

$$
f = \sum_{i,j} \iota_i^\calk \circ f_{ij} \circ \pi_j^\calh.
$$

Function composition then becomes a form of "matrix multiplication":

::: {#thm-composition}
Suppose that we have a pair of functions

$$
\bigoplus_j \calh_j \xrightarrow{f} \bigoplus_i \calk_i \xrightarrow{g} \bigoplus_k \mathcal{L}_k,
$$

between (finite) orthogonal direct sums of finite-dimensional Hilbert spaces. Then the component functions of the composition $g\circ f$ are given by

$$
(g\circ f)_{kj} = \sum_i g_{ki} \circ f_{ij},
$$

for all $j$ and $k$.
:::

Continuing with our function $f: \calh \to \calk$, one may prove that if $f$ is differentiable, then so too are each of the component functions $f_{ij}$, and moreover, we have the fundamental link between the derivative of $f$ and the derivatives of its component functions:

::: {#thm-component-derivative}
Let $f: \bigoplus_j \calh_j \to \bigoplus_i \calk_i$ be a differentiable function between (finite) orthogonal direct sums of finite-dimensional Hilbert spaces. Then each component function $f_{ij}$ is differentiable, and the derivative of $f$ is given by

$$
f'(x) = \sum_{i,j} \iota_i^\calk \circ f_{ij}'(x) \circ \pi_j^\calh,
$$

for all $x \in \calh$.
:::

This leads us to the following generalization of the definition of partial derivatives:

::: {#def-partial-derivative-direct-sum}
Let $f: \bigoplus_j \calh_j \to \bigoplus_i \calk_i$ be a differentiable function between (finite) orthogonal direct sums of finite-dimensional Hilbert spaces. For each $i$ and $j$, we define the *(i,j)-th partial derivative* of $f$ at a point $x \in \calh$ to be the linear map

$$
\frac{\partial f_i}{\partial \calh_j}(x) = f_{ij}'(x): \calh_j \to \calk_i.
$$
:::
