...
::: {#def-algebra-gen-rv}
Let $X_1,X_2,\ldots,X_n$ be random vectors on a finite set $\Omega$. The *algebra generated* by $X_1,X_2,\ldots,X_n$ is the algebra of sets in $\Omega$, denoted by $\sigma(X_1,X_2,\ldots,X_n)$, whose atoms are all nonempty intersections of the form 
$$
...
In fact, if we are given a finite set of random variables $X_1,X_2,\ldots,X_n$ as in the theorem, then we may form the random vector
...
where the integral on the right is the Lebesgue integral with respect to the probability measure $P$. In the finite setting, this integral reduces to the familiar weighted average:

$$
\int_\Omega X \ dP = \sum_{i=1}^n x_i P(A_i),
$$

where $A_1, A_2, \ldots, A_n$ are the level sets of $X$ and $x_i$ is the value of $X$ on $A_i$ for each $i$. Note that in order for the probability $P(A_i)$ to be defined, we must have $A_i \in \calF$, which is guaranteed if $X$ is $\calF$-measurable.
...
One should check that $L^2(\calF)$ is indeed a vector subspace of the vector space of all random variables on $\Omega$, which amounts to checking that sums and scalar multiples of $\calF$-measurable random variables are $\calF$-measurable. To see that it is closed under addition, let $X,Y \in L^2(\calF)$, so that $X$ and $Y$ are both $\calF$-measurable. Then, for any $a \in \bbr$, we have

$$
(X+Y)^{-1}(a) = \bigcup_{b \in \bbr} \left( X^{-1}(b) \cap Y^{-1}(a-b) \right),
$$

and the right-hand side is in $\calF$ since $\calF$ is an algebra. Thus, $X+Y$ is $\calF$-measurable, so $X+Y \in L^2(\calF)$. A similar argument shows that scalar multiples of $\calF$-measurable random variables are again $\calF$-measurable, proving that $L^2(\calF)$ is a vector subspace.

Besides inducing the equivalence classes under almost sure equality, the probability measure $P$ does not factor into the definition of $L^2(\calF)$. But it does define an inner product on this space, via expectation:
...
One should check that $\langle -, -\rangle$ satisfies the properties of an inner product: linearity in the first argument, symmetry, and positive-definiteness. Linearity and symmetry follow directly from the familiar properties of the expectation operator. To see positive-definiteness, note that for any $X \in L^2(\calF)$, we have

$$
\langle X, X \rangle = E(X^2) = \sum_{i=1}^m x_i^2 P\left(A_i\right) \geq 0
$$

using ([-@eq-rv-indicator-expansions]). If equality held, then we must have $x_i^2 P\left(A_i\right)=0$ for every $i$. So, if $x_i\neq 0$, we must have $P\left(A_i\right) = 0$, which proves that $X$ is almost surely equal to the zero random variable. This proves positive-definiteness.
