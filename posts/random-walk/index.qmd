...
csl: /Users/johnmyers/dev/johnmyers-phd/aux-files/american-mathematical-society-numeric.csl
image: "thumbnail.png"
open-graph:
    title: "SigAlg I: IID processes and random walks"
...
SigAlg is a new open-source Python library designed for working with measure-theoretic probability theory and stochastic processes. Unlike many statistical computing libraries that prioritize computational efficiency or simplify the mathematical formalism, SigAlg aims to provide an API with high fidelity to the underlying mathematics. The goal is to make manipulating probability spaces, random variables, $\sigma$-algebras, and stochastic processes as natural as writing equations on a whiteboard or working through proofs on paper. Whether you're defining random variables, computing conditional expectations, or constructing filtrations, SigAlg's object-oriented design mirrors the mathematical structures you already know.

In this first post of a series on SigAlg, we explore two fundamental building blocks of stochastic process theory: independent and identically distributed (IID) processes and random walks. Through concrete examples with code, we'll see how SigAlg allows us to construct these objects, inspect their probability measures and natural filtrations, visualize information flow, and simulate trajectories—all while maintaining mathematical rigor and clarity. By the end, you'll have a sense of how SigAlg can serve as both a computational tool and a pedagogical aid for understanding stochastic processes.
...
# Add the initial state to the increment process
