...
Throughout these notes, we work with a finite-dimensional Hilbert space $\calh$, which for us just means that $\calh$ is a finite-dimensional vector space over the field $\mathbb{C}$, with an inner product

$$
\langle -, - \rangle : \calh \times \calh \to \mathbb{C}
$$

that is linear in the first argument and conjugate-linear in the second argument. Note that this is the *opposite* of the usual convention in physics, where the inner product is conjugate-linear in the first argument.

The *algebraic dual* of $\calh$, denoted $\calh^\ast$, is the $\bbc$-vector space of all linear functionals $\varphi: \calh\to \bbc$. Since $\calh$ is finite-dimensional, we need not be careful with the distinction between linear functionals and *continuous* linear functionals. The vector space operations in $\calh^\ast$ are the usual pointwise ones.

Every vector $w\in \calh$ defines a linear functional $w^\ast \in \calh^\ast$ by the formula

$$
w^\ast(v) = \ang{v,w}, \quad \forall v\in \calh.
$$

This yields a transformation

$$
\Phi: \calh \to \calh^\ast, \quad w\mapsto w^\ast,
$$

that we call the *Riesz transformation*. The reader may check that $\Phi$ is $\bbc$-linear. In fact, it is an isomorphism of vector spaces, as the following theorem shows:


::: {#thm-riesz}
## Riesz Representation Theorem — Finite-Dimensional Version

Let $\varphi$ be a linear functional on a finite-dimensional Hilbert space $\calh$. Then there is a unique vector $w_\varphi\in \calh$ such that $w_\varphi^\ast = \varphi$. In particular, the Riesz transformation is an isomorphism of vector spaces between $\calh$ and its dual $\calh^\ast$.
:::
...
Since the Riesz transformation is an isomorphism, we may use it to transport the inner product from $\calh$ to $\calh^\ast$ by defining

$$
\langle \varphi, \psi \rangle = \langle w_\varphi, w_\psi \rangle, \quad \forall \varphi, \psi \in \calh^\ast.
$$

Equipped with this inner product, $\calh^\ast$ becomes a Hilbert space in its own right, and the Riesz transformation $\Phi$ becomes an *isometric* isomorphism.
...
3. A linear functional $\varphi: C(\calx) \to \bbc$ is called *positive* if $\varphi(f) \geq 0$ whenever $f\in C(\calx)$ is a function with its image in $[0,\infty)$; it is called *normalized* if $\varphi(\id)=1$, where $\id: \calx \to \bbc$ is the constant function that sends all of $\calx$ to $1\in \bbc$.
